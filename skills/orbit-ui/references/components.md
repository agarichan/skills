...
| `code-viewer` | registry:ui | 構文ハイライト + Diff ビュワー。Shiki ベース、CodeBlock / StyledCodeBlock / DiffViewer の 3 種 | tokens, icon, truncate | shiki, diff |
| `log-viewer` | registry:ui | 構造化ログ表示。時刻・レベル色分け・フィルタ・自動追従・コピー・折返し・拡大。ERROR 行ハイライト | tokens, icon, input, toggle, time-display | @tanstack/react-virtual |
| `data-table` | registry:ui | 列定義ベーステーブル。仮想化・無限スクロール・sticky ヘッダ / sticky-left・ソート・行選択・skeleton・expand・Stats | tokens, icon, tooltip, compact-number, toggle | @tanstack/react-virtual |
